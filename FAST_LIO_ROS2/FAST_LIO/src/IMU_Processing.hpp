...
ImuProcess::ImuProcess()
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1), last_lidar_end_time_(0.0)
{
  init_iter_num = 1;
  Q = process_noise_cov();
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  if (b_first_frame_)
  {
    ...
    V3D first_acc(imu_acc.x, imu_acc.y, imu_acc.z);
    if (first_acc.norm() < 1.0)
    {
      // Simulator IMU not publishing valid acceleration yet, skip this frame
      return;
    }
    Reset();
    N = 1;
  ...
  for (const auto &imu : meas.imu)
  {
    ...
    if (cur_acc.norm() < 1.0)
    {
      // Ignore uninitialized simulation steps
      continue;
    }
  ...
  if (N <= 1)
  {
    // No valid non-zero IMU messages processed yet
    return;
  }
